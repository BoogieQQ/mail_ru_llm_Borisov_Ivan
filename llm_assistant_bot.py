...
/start старт бота
/model выбор модели
/check_model посмотреть, как модель сейчас загружена
...
def model(message):
    ...
    btn2 = types.KeyboardButton("GPT (не доступно)")
    btn3 = types.KeyboardButton("Llama (не доступно)")
    markup.add(btn1, btn2, btn3)
    bot.send_message(message.from_user.id, "Выберите модель для генерации", reply_markup=markup)


@bot.message_handler(commands=['check_model'])
...
def get_text_messages(message):
    ...
    if message.text in ['StatLM', 'GPT', 'Llama']:
        print(f'Find bot name <{message.text}>')
